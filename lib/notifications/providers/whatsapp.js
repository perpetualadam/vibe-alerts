...
export class WhatsAppProvider extends NotificationProvider {
  ...
  static setupGuide = [
    'Enter the WhatsApp number that should receive alerts.',
    'Configure WHATSAPP_ACCESS_TOKEN and WHATSAPP_PHONE_NUMBER_ID on the server.',
    'The recipient must have opted in to receive business messages.',
  ];
  ...
  async send(context) {
    ...
    if (!whatsappAccessToken || !whatsappPhoneNumberId) {
      return {
        success: false,
        error: 'WhatsApp provider not configured on server',
        retryable: false,
      };
    }
