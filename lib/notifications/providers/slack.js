...
function formatSlackFieldValue(key, value) {
  const callable = formatCallablePhoneValue(
    key,
    value,
    (display, href) => `<${href}|${display}>`
  );
...
function formatSlackMessage(payload, sourceLabel = 'Website Form') {
  const fields = Object.entries(payload).map(([key, value]) => ({
    type: 'mrkdwn',
    text: `*${formatFieldLabel(key)}:*\n${formatSlackFieldValue(key, value)}`,
  }));
...
export class SlackProvider extends NotificationProvider {
  ...
  async healthCheck(context) {
    // Slack uses per-tenant Incoming Webhook URLs — platform is always ready.
