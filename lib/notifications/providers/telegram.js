...
/** Escape ) and \ inside MarkdownV2 inline-link URLs */
function escapeMarkdownV2Url(url) {
  return String(url).replace(/([)\\])/g, '\\$1');
}

function formatTelegramFieldValue(key, value) {
  const callable = formatCallablePhoneValue(key, value, (display, href) => {
    return `[${escapeMarkdownV2(display)}](${escapeMarkdownV2Url(href)})`;
  });
...
function formatTelegramMessage(payload, sourceLabel = 'Website Form') {
  ...
  for (const [key, value] of Object.entries(payload)) {
    lines.push(
      `*${escapeMarkdownV2(formatFieldLabel(key))}:* ${formatTelegramFieldValue(key, value)}`
    );
  }
...
export class TelegramProvider extends NotificationProvider {
  ...
  static setupGuide = [
    ...
    'Copy the numerical ID given to you and paste it below, then click Save Telegram.',
    'Open your VibeAlerts bot in Telegram and tap Start (or send /start) before testing.',
    'For team groups: add the bot to the group (group IDs start with -).',
  ];
