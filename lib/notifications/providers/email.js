...
export class EmailProvider extends NotificationProvider {
  ...
  static platformUnavailableMessage =
    'Email alerts are not enabled on VibeAlerts yet. Use Telegram, Slack, Discord, or Teams, or contact support to request email delivery.';
  static setupGuide = [
  ...
  async healthCheck(context) {
    if (!this.isPlatformReady()) {
      return {
        healthy: false,
        provider: this.id,
        message: 'Resend credentials (RESEND_API_KEY / RESEND_FROM_EMAIL) are not configured',
      };
    }
