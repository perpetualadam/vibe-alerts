...
export class EmailProvider extends NotificationProvider {
  ...
  static setupGuide = [
    ...
    'Ensure RESEND_API_KEY and RESEND_FROM_EMAIL are configured on the server.',
    'If the domain uses Cloudflare Email Routing, merge Resend into the root SPF record (see docs/CLOUDFLARE.md).',
  ];
  ...
  async send(context) {
    ...
    if (!resendApiKey || !resendFromEmail) {
      return {
        success: false,
        error: 'Email provider not configured on server (RESEND_API_KEY, RESEND_FROM_EMAIL)',
        retryable: false,
      };
    }
