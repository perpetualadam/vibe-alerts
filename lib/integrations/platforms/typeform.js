...
export class TypeformIntegration extends PlatformIntegration {
  ...
  static setupSteps = [
    ...
    'Paste your VibeAlerts Webhook URL and enable the webhook.',
    'Add custom header X-VibeAlerts-Key with your API key (Typeform “secret” / custom headers).',
    'Add custom header X-VibeAlerts-Platform: typeform (or proxy via Zapier/Make if headers are unavailable).',
    'Use Send Test Notification in the VibeAlerts dashboard, then submit a real Typeform response.',
    'Typeform delivers form_response JSON — VibeAlerts maps answers to flat alert fields automatically.',
  ];
