 ...
 * @param {Record<string, unknown>} params.config
 * @param {boolean} params.enabled
 * @param {string|null} [params.connectedAt] - Explicit connected_at; pass null to clear
 * @param {boolean} [params.touchConnectedAt=true] - When true and config has values, set connected_at to now
 */
export async function upsertChannelConfig(
  supabase,
  { userId, channel, config, enabled, connectedAt, touchConnectedAt = true }
) {
  const row = {
    user_id: userId,
    channel,
  ...
  if (connectedAt !== undefined) {
    ...
    touchConnectedAt &&
    config &&
    Object.values(config).some((v) => v != null && String(v).trim() !== '')
  ) {
