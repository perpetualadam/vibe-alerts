...
export function buildLlmsTxt() {
  ...
  const lines = [
    ...
    '',
    '## Summary',
    `${SITE.name} is a multi-tenant B2B SaaS that receives website form submissions via webhooks and forwards them to notification channels (Telegram, Email, Slack, Microsoft Teams, WhatsApp).`,
    '',
    '## Primary use case',
...
export function buildLlmsFullTxt() {
  ...
  return [
    ...
    'Telegram (chat ID + platform bot), Email (Resend), Slack (incoming webhook URL), Teams (incoming webhook URL), WhatsApp (Meta Cloud API).',
    '',
    '## Pricing model',
