...
export async function runWizardConnectionTest(userId, options = {}) {
  ...
  if (mode === 'verify_site') {
    ...
    const match = (events || []).find((event) => {
      ...
      // WP form plugins may still report umbrella wordpress while source matches
      const payloadSource = event.received_payload?.source || event.received_payload?._vibealerts_source;
      if (payloadSource && String(payloadSource).replace(/-/g, '_') === integrationId) return true;
      if (
        integrationId !== 'wordpress' &&
        platform === 'wordpress' &&
        payloadSource &&
        String(payloadSource).toLowerCase().includes(integrationId.replace(/_/g, '-').split('_')[0])
      ) {
        return true;
      }
  ...
  // Simulate: platform-shaped payload through the real webhook processor
  const { buildIntegrationTestPayload } = await import('@/lib/integrations/test-payloads');
  const testPayload = buildIntegrationTestPayload(integrationId);
  testPayload._vibealerts_wizard_test = true;
