...
export async function processWebhookRequest({ token, rawBody, headers, sourceIp }) {
  ...
  // 9. Optional AI Lead Intelligence — queue analysis so the HTTP response stays fast
  let aiJobId = null;
  ...
  try {
    ...
    if (aiSettings.enabled && isAiPlatformConfigured() && event?.id) {
      const queued = await enqueueAndScheduleAiAnalysis({
        userId: profile.id,
        webhookEventId: event.id,
        payload: rulesResult.payload,
        notifyAfter: aiSettings.includeInNotifications,
        heuristicSpamScore: spam.score,
        deliveryContext: {
      ...
      aiJobId = queued.jobId;
      deferNotification = Boolean(queued.jobId && queued.deferredNotification);
    }
  } catch (err) {
    logger.warn('AI lead intelligence enqueue skipped', {
      userId: profile.id,
      error: err.message,
    });
  }

  // 10. Fan-out via NotificationService (unless deferred until AI completes)
  //     Webhook → Rules → (optional AI queue) → NotificationService
  let deliveryResults = [];
  if (deferNotification) {
    ...
    logger.info('Notification deferred until AI analysis completes', {
      userId: profile.id,
      eventId: event?.id,
      aiJobId,
    });
    ...
    const deliverySummaryInner = deliveryResults.map((r) => ({
      channel: r.channel,
      success: r.success,
      error: r.error ?? null,
    }));
    ...
    await updateWebhookEvent(supabase, event?.id, {
      ...
      delivery_summary: deliverySummaryInner,
      rules_applied: rulesApplied,
  ...
  return jsonResponse(
    200,
    {
      ...
      ...(aiJobId ? { aiJobId, aiQueued: true } : {}),
      ...(deferNotification
        ? { message: 'Lead accepted. Notifications will send after AI analysis.' }
        : {}),
      ...(failures.length
