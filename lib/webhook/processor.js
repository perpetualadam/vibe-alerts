...
export async function processWebhookRequest({ token, rawBody, headers, sourceIp }) {
  ...
  // 8. Optional AI Lead Intelligence — queue analysis so the HTTP response stays fast
  let aiJobId = null;
  ...
  // 9. Fan-out via NotificationService (unless deferred until AI completes)
  ...
  // 10. Update last webhook timestamp + monthly usage metering
  await supabase
    .from('user_settings')
